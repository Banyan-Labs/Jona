...
import { JobService } from "@/utils";
import { supabase } from "@/lib/supabaseClient";
import type { AuthUser, Job } from "@/types/index";
...
export type JobStatus = "applied" | "pending" | "interview" | "rejected" | "offer";
  ...
  const handleToggleSaved = async (jobId: string, currentSaved: boolean) => {
    try {
     await upsertStatus({
      ...
      setJobs((prev) =>
        prev.map((j) =>
          j.id === jobId ? { ...j, saved: !currentSaved } : j
        )
      );
    } catch (error) {
