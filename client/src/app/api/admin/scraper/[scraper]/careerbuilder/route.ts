// client/src/app/api/admin/scraper/careerbuilder/route.ts
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await supabaseAdmin.from("admin_audit_logs").insert({
      ...
      action: "careerbuilder_scraper_started",
      entity_type: "system",
      entity_id: logId,
    ...
    const scraperResult = await startCareerBuilderScraperProcess(
     scraperRequest,
      logId!,
      adminUser
    );
    ...
    console.error("CareerBuilder scraper initialization error:", error);
    ...
    return createErrorResponse(
      error instanceof Error ? error.message : "Failed to start CareerBuilder scraper"
    );
  }
}
...
async function startCareerBuilderScraperProcess(
  ...
  return new Promise<any>((resolve) => {
    try {
        ...
        : ""; 
      const scraperArgs = [
        "server/app/scraper/run_careerbuilder.py",
         config.location,
      ...
      console.log("Starting CareerBuilder scraper with args:", scraperArgs);
      ...
      const timeout = setTimeout(() => {
        console.log(`CareerBuilder scraper ${logId} timeout reached, terminating`);
      ...
      scraper.stderr.on("data", (data: Buffer) => {
        ...
        console.error(`CareerBuilder scraper ${logId} error:`, errorStr);
      ...
      scraper.on("close", async (code: number) => {
        ...
        console.log(`CareerBuilder scraper ${logId} finished: code=${code}, found=${jobsFound}, saved=${jobsSaved}`);
      ...
      scraper.on("error", async (error: Error) => {
        clearTimeout(timeout);
        console.error(`Failed to start CareerBuilder scraper ${logId}:`, error);

        await updateScrapingLogOnError(logId, error);

        resolve({
          success: false,
          error: `Failed to start CareerBuilder scraper: ${error.message}`,
          log_id: logId,
          scraper_name: "careerbuilder",
        });
      });
    } catch (error) {
      console.error(`Error initializing CareerBuilder scraper ${logId}:`, error);
      resolve({
        success: false,
...
async function updateFinalScrapingStatus(
  ...
  try {
    ...
    await supabaseAdmin.from("admin_audit_logs").insert({
      admin_user_id: adminUser.id,
      admin_email: adminUser.email,
      action: isSuccess ? "careerbuilder_scraper_completed" : "careerbuilder_scraper_failed",
      entity_type: "system",
      entity_id: logId,
      new_values: {
        jobs_found: jobsFound,
