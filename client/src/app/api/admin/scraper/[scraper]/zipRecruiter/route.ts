// client/src/app/api/admin/scraper/ziprecruiter/route.ts
import { NextRequest, NextResponse } from "next/server";
import { spawn, type ChildProcessWithoutNullStreams } from "child_process";
import supabaseAdmin from "@/lib/supabaseAdmin";
import { validateAdminAuth, createAuthResponse } from "@/lib/supabase/auth-middleware";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const rateLimitKey = `ziprecruiter-scraper:${adminUser.id}`;
    ...
    await supabaseAdmin.from("admin_audit_logs").insert({
      ...
      action: "ziprecruiter_scraper_started",
      entity_type: "system",
      entity_id: logId,
    ...
    const scraperResult = await startZipRecruiterScraperProcess(
     scraperRequest,
      logId!,
      adminUser
    );
    ...
    console.error("ZipRecruiter scraper initialization error:", error);
    ...
    return createErrorResponse(
      error instanceof Error ? error.message : "Failed to start ZipRecruiter scraper"
    );
  }
}
...
async function startZipRecruiterScraperProcess(
  ...
  return new Promise<any>((resolve) => {
    try {
        ...
        : "";
      const scraperArgs = [
        "server/app/scraper/run_ziprecruiter.py",
       config.location,
      ...
      console.log("Starting ZipRecruiter scraper with args:", scraperArgs);
      ...
      const timeout = setTimeout(() => {
        console.log(`ZipRecruiter scraper ${logId} timeout reached, terminating`);
      ...
      scraper.stderr.on("data", (data: Buffer) => {
        ...
        console.error(`ZipRecruiter scraper ${logId} error:`, errorStr);
      ...
      scraper.on("close", async (code: number) => {
        ...
        console.log(`ZipRecruiter scraper ${logId} finished: code=${code}, found=${jobsFound}, saved=${jobsSaved}`);
      ...
      scraper.on("error", async (error: Error) => {
        clearTimeout(timeout);
        console.error(`Failed to start ZipRecruiter scraper ${logId}:`, error);

        await updateScrapingLogOnError(logId, error);

        resolve({
          success: false,
          error: `Failed to start ZipRecruiter scraper: ${error.message}`,
          log_id: logId,
          scraper_name: "ziprecruiter",
        });
      });
    } catch (error) {
      console.error(`Error initializing ZipRecruiter scraper ${logId}:`, error);
      resolve({
        success: false,
...
async function updateFinalScrapingStatus(
  ...
  try {
    ...
    await supabaseAdmin.from("admin_audit_logs").insert({
      admin_user_id: adminUser.id,
      admin_email: adminUser.email,
      action: isSuccess ? "ziprecruiter_scraper_completed" : "ziprecruiter_scraper_failed",
      entity_type: "system",
      entity_id: logId,
      new_values: {
        jobs_found: jobsFound,
